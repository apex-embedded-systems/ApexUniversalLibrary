...
struct espec_ect2_slot
{
	SYS_TYPE	type;
	double		temperature_set_point_celcius;
	double		time_soak_minutes;
	double		time_measure_minutes;
};
typedef struct espec_ect2_slot espec_ect2_slot_type;

AUL_DLL_API int Espec_Ect2_Slot_Build(	double temperature_set_point_celcius,
										double time_soak_minutes,
										double time_measure_minutes,
										espec_ect2_slot_type * slot
									);

AUL_DLL_API int Espec_Ect2_Frame_Slot_Count_Max( int board, int component_instance, int * count_max );

AUL_DLL_API char * Espec_Ect2_Slot_String( espec_ect2_slot_type slot );


// This one might have to use an internal timer and then spawn other tasks using select() or poll()
// intended to be non-blocking.
AUL_DLL_API int Espec_Ect2_Transmit_Frame( int board, int component_instance, espec_ect2_slot_type * list, size_t list_qty );

AUL_DLL_API int Espec_Ect2_Transmit_Slot( int board, int component_instance, espec_ect2_slot_type * slot );

// returns a buffer of temperature data.
AUL_DLL_API int Espec_Ect2_Receive( int board, int component_instance, size_t temp_qty, double * temp_celcius );

=======================
OPERATIONS

The general behavior would be the similar to the LTC2449 form.
