...
/*********************************************************************************/
IO_ID_ENUM IO_ID_Get( void ) {	return IO_ID_WIN32_LIB_SIMULATOR;  }
/*********************************************************************************/
...
/*********************************************************************************/
static int IO_WLS_Interrupt_Disable( struct io_device_dataset * ds )
{
	(void) ds;
	return -EC_IO_NOT_IMPLEMENTED;
}
/*********************************************************************************/
static int IO_WLS_Interrupt_Enable( struct io_device_dataset * ds, io_int_handler handler, void * private_data )
{
	(void) ds;
	(void) handler;
	(void) private_data;
...
static struct io_definition io_wls_definition[DEVICE_QTY];
/*********************************************************************************/
static int IO_WLS_Ioctl( struct io_device_dataset * ds, unsigned int command, unsigned long argument )
{
	int		error_code;
	(void)   ds;
	(void)   command;
	...
	{	/* emulate the driver */
		case CMD_AULDRV_NAME:
			{
				...
				{ error_code = EC_IO_COPY_TO_USER; goto IO_WLS_Ioctl_Error;  }
				else
				{ error_code = SUCCESS;  }
			}
			break;
		case CMD_AULDRV_DEVICE_LIST_GET:
			{ /* copy the io_definitions into place */
				...
				count = 0;
				while( IOC_PHYSICAL_ID_NULL != io_wls_definition[count].physical_id )
				{
					Mem_Copy( &(device_list.region[count]), &(io_wls_definition[count]), sizeof( struct io_definition ) );
					count++;
				}
		...
		case CMD_AULDRV_DEBUG_ACTIVE_GET:
			{ /* return zero if IO_DEBUG is not defined, otherwise non-zero */
				...
				{ error_code = EC_AULDRV_COPY_TO_USER;	goto IO_WLS_Ioctl_Error;  }
			}
			break;
	}
	if ( error_code ) goto IO_WLS_Ioctl_Error;
#if defined( IO_DEBUG_LOG )
...
/*********************************************************************************/
static int IO_WLS_Open( struct io_device_dataset * ds )
{
	int	error_code;
#if defined( IO_DEBUG_LOG )
	Debug_Log( IO_DEBUG_LOG "%s: entry\n", __func__ );
	...
	{	error_code = EC_IO_OPEN;	goto IO_WLS_Open_Error;	}
#if defined( IO_DEBUG_LOG )
	Debug_Log( IO_DEBUG_LOG "%s: exit success\n", __func__ );
#endif
...
/*********************************************************************************/
static int IO_WLS_Close( struct io_device_dataset * ds )
{
...
/*********************************************************************************/
#if defined( IO_DEBUG )
static int IO_WLS_Validity( int iod, size_t virtual_address, size_t data_bytes )
{
	int error_code;
...
#endif
/*********************************************************************************/
static int IO_WLS_Terminate( void )
{
#if defined( IO_DEBUG_LOG )
	Debug_Log( IO_DEBUG_LOG "%s: entry\n", __func__ );
#endif
...
/*********************************************************************************/
static int IO_WLS_Initialize( void )
{
#if defined( IO_DEBUG_LOG )
	Debug_Log( IO_DEBUG_LOG "%s: entry\n", __func__ );
#endif
...
/*********************************************************************************/
static int IO_WLS_Definition_Available( void )
{
	return SUCCESS;
}
/*********************************************************************************/
static int IO_WLS_Definition_Get( struct io_definition * io_definition )
{
/* search for matching acronym.  If that fails, then search by minor number.  */
	...
	index = 0;
	acronym = io_wls_definition[index].acronym;
	while ( NULL != acronym )
	{
		...
		{	/* copy selected values into the user structure */
			Mem_Copy( io_definition, &(io_wls_definition[index]), sizeof( struct io_definition ) );
			return SUCCESS; 
		}
		index++;
		acronym = io_wls_definition[index].acronym;
	}
/* IO_WLS_Definition_Get_Error: */
...
io_target_type io_target_definition =
{
	...
	IO_WLS_Close,
	IO_WLS_Ioctl,
	IO_WLS_Interrupt_Enable,
	IO_WLS_Interrupt_Disable,
	IO_WLS_Definition_Available,
	IO_WLS_Definition_Get,
#if defined( IO_DEBUG )
	IO_WLS_Validity,
#endif
	/* wls = win32 library simulator */
	IO_TARGET_NAME_SUFFIX( MODULE_IO_NAME, "wls" ),
	IO_MODULE_REV,
	IO_COMMAND_REV,
	IO_STATUS_REV
};
