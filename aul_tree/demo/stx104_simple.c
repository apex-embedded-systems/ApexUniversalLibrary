...
/***************************************************************************/
#define STX104_SIMPLE_REV  "$Date: 2015-01-12 14:07:46 -0600 (Mon, 12 Jan 2015) $"
...
/*****************************************************************************//**
 * @def STX104_SIMPLE_DEBUG_MESSAGE
 *
 * <b>CUSTOMER_TODO:</b>
 *
...
*/
#define STX104_SIMPLE_DEBUG_MESSAGE  _DEBUG

/*****************************************************************************//**
 * @def STX104_SIMPLE_USE_INTERRUPTS
 *
 * <b>CUSTOMER_TODO:</b>
 *
...
*/
/*#define STX104_SIMPLE_USE_INTERRUPTS  STX104_CONFIG_INTERRUPTS_ACTIVE*/

/*****************************************************************************//**
 * @def TIMER_PARAMETERS_IMPROVE_DYNAMIC_RANGE
 *
 * <b>CUSTOMER_TODO:</b>
 *
 * If enabled this will use the improved timing parameters dynamic range and is
 * the newer preferred method.  Timing parameters are passed as doubles rather
 * than the UINT32.  It is sometimes easier to express in floating point than
 * in integer values.
 */
#define TIMER_PARAMETERS_IMPROVE_DYNAMIC_RANGE 1
/*****************************************************************************//**
...
*/
  ...
  #define AI_FRAME_TIME_SEC  0.5
#else
/*                         mmmuuunnn     m - milliseconds, u - microseconds, n - nanoseconds */
  #define AI_FRAME_TIME_NS 500000000
#endif
/*****************************************************************************//**
...
/*****************************************************************************//**
 * @def DOUT3_TOGGLE_EACH_PERIODIC_WRITE_CYCLE
 *
 * <b>CUSTOMER_TODO:</b>
 *
 * Comment out to not toggle DOUT3 each time we write data to a file.  If
 * there is no data to write, then do not toggle.
 *
 * If uncommented, then DOUT3 (J7.5) will be toggled each time analog input
...
*/
void STX104_Simple_Menu_AI_Data_Readout( struct demo_stx104_data * ds )
{
	int 		error_code;
	int   		board;
	BOOL  		data_available;
...
static void CallBack_Source_A( void * result )
{
	...
	ds = (struct demo_stx104_data *) event_data->user_defined_dataset;
...
#if defined( STX104_SIMPLE_USE_INTERRUPTS )
static const char stx104_simple_interrupt_handler_name[]  = "CallBack_Source_A";
static const char stx104_simple_interrupt_data_set_name[] = "stx104_simple_data_private";
#endif
 ...
 * @param  ds	   DataSet.  See demo_stx104_data for further details.
 * @return If successful, returns SUCCESS (0), otherwise an error-code.
 */
int STX104_Simple_Configuration( char * prefix, struct demo_stx104_data * ds )
{
	int 				 			error_code;
	struct stx104_configuration * 	cfg;
	FILE *							out_file;
	int								board;
	...
	cfg->name_interrupt_callback_a      = stx104_simple_interrupt_handler_name;
	cfg->name_interrupt_data_set_a      = stx104_simple_interrupt_data_set_name;
#else
 ...
 *TODO
 */
static int STX104_Simple_Menu_Interrupt( struct menu_command_params * p )
{
	int 				error_code;
	BOOL	 			value_prev;
	BOOL				value;
 ...
 *TODO
 */
static struct menu_data stx104_simple_data_list[] =
{
 ...
 * @return Always returns SUCCESS (0)
 */
int STX104_Simple_Main_Stop_Request( void * private_data )
{
	struct demo_stx104_data * 	ds;
...
int STX104_Simple_Main_Loop( void * private_data )
{
	int 						error_code;
	int    						board;
	FILE * 						out_file;
 ...
 #endif
	error_code = Menu_Command( stx104_simple_menu_list, board, in_file, out_file, ds );
	/* display status and updated information */
	Menu_Data( stx104_simple_data_list, 0, board, out_file, ds );
#if defined( DOUT3_TOGGLE_EACH_PERIODIC_WRITE_CYCLE )
	ds->dout3_state = !ds->dout3_state;
	STX104_DO_Set( board, STX104_DO3, ds->dout3_state );
#endif
...
int main( int argc, char* argv[] )
{
	...
	error_code = STX104_Simple_Configuration( prefix, &demo_stx104_data );
	if ( error_code ) goto Main_Error;
	fprintf( out_file, "Main: STX104_Simple_Configuration() complete\n" );

	/***** menu initialization *****/
	Menu_Initialization( stx104_simple_menu_list );

	/***** Main loop here *****/
	fprintf( out_file, "Main: process loop begin\n\n" );
	Menu_Display( stx104_simple_menu_list, board, out_file, ds );
	...
	{  /* process any incoming menu commands */
		error_code = STX104_Simple_Main_Loop( ds );
	} while ( !error_code );
#endif
