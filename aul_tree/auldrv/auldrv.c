...
#endif
/* <EGS_BEGIN>*/
#if defined( DEVICE_STX104_EGS )

//must be defined in same scope!
static const char auldrv_stx104egs[]  = DEVICE_STX104_NAME_EGS;

if( 0 == String_Compare( auldrv_stx104egs, auldrv_requested_region_type[index_device],
						 String_Length( auldrv_stx104 )                                ) )
{
	d->physical_id = IOC_CREATE_LOGICAL_ID_DEVICE( DEVICE_STX104 );

	// adjust byte width here.
PDEBUG( "STX104EGS: d->physical_id = %d\n", d->physical_id );
}
#endif
/* <EGS_END>*/
PDEBUG( "%s exit success.\n", __func__ );
