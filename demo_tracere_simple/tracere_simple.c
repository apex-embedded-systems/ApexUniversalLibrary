 ...
 * Tracer-E demonstration
 *
 * @mainpage   Tracer-E Simple
 ...
 * REVISION HISTORY
 *
 * 	January  28, 2015 - reused from STX104_Simple.
 *
 *
 ...
 * <b>DESCRIPTION</b>
 *
 * This application is intended for simple Tracer-E I/O testing.
 *
 * All of the parameters are set via macros below.
 *
...
/***************************************************************************/
#define TRACERE_SIMPLE_REV  "$Date: 2015-02-02 11:08:17 -0600 (Mon, 02 Feb 2015) $"
...
int Tracere_Simple_Configuration( char * prefix, struct demo_tracere_data * ds )
{
	...
	Demo_Tracere_Config_Clear( cfg );

	/**** initialize interrupts and triggers ****/
	error_code = Demo_Tracere_Config_Initialize( board, prefix, out_file, cfg );
	if ( SUCCESS != error_code ) return (int) error_code;

	/*********************************************************************/
	/**** set our configuration parameters here for this application  ****/

	cfg->dio.i8255m0_cfg[0].port_directions = I8255M0_PDIR_AOUT_BOUT_COUT;
	//cfg->dio->i8255m0_cfg[0].port_directions = I8255M0_PDIR_AOUT_BOUT_COUT;
   ...
   error_code = Demo_Tracere_Config_Commit( board, prefix, out_file, cfg );
   if ( SUCCESS != error_code ) return (int) error_code;
   return SUCCESS;
}
...
{   0,     4,  	NULL,						"---- digital i/o  I8255M0 -----"						},  /* display only 				 */
{ 's',	   0,	Menu_DO_Bit_Set,			"set digital out bit"			 						},
{ 'c',	   0,	Menu_DO_Bit_Clear,			"clear digital out bit"			 						},
{ 'i',	   0,	Menu_DI_Bit,				"read digital input bit"		 						},
{ 'a',     0,   Menu_DI_Bit_All,			"read all digital input bits"							},
{ 'w',     0,   Menu_DIO_Port_Write,		"write digital output port"								},
{ 'r',     0,   Menu_DIO_Port_Read,			"read digital input port"								},
#if(0)
{ '1',     0,   Menu_DIO_Port_Dir,			"port directions I8255M0  J9"							},
{ '2',     0,   Menu_DIO_Port_Dir,			"port directions I8255M0  J12"							},
#endif
{   0,     4,  	NULL,						"---- counter/timer I8254 -----"						},  /* display only 				 */
...
static struct menu_data tracere_simple_data_list[] =
{
	...
	{	0,			0,		Demo_Tracere_Data_Timer_Tick,	  0,				NULL	  		},
#if defined( STX104_SIMPLE_USE_INTERRUPTS )
...
//	{ /* aside */
//		error_code = AUL_DO_Set( board,  9, true );
//		error_code = AUL_DO_Set( board, 11, true );
//		/* terminal:
//		 * $ dmesg
//		 *      ...
//		 *      [ 4623.376577] DEBUG: IO_Ioctl: entry
//				[ 4623.376585] DEBUG: IO_Xfr_Command: entry
//				[ 4623.376600] DEBUG: IO_Write_Repeat_U8: mem[0x0311]<=0x02  <-- first bit
//				....
//				[ 4632.574406] DEBUG: IO_Write_Repeat_U8: mem[0x0311]<=0x0A  <-- second bit
//		 *      ...
//		 */
//		//fprintf( out_file, "Main: dio_port[0] = 0x%02X\n", byte_value );
//	}
//	{
//		struct menu_command_params  mcp;
//
//		mcp.board    = board;
//		mcp.channel  = 0;
//		mcp.in_file  = stdin;
//		mcp.out_file = stdout;
//
//		error_code = Menu_DI_Bit_All( &mcp );
//		return -error_code;
//
//	}
...
//		p.board    = board;
//		p.channel  = 0;
//		p.in_file  = ds->in_file;
//		p.out_file = ds->out_file;
//		error_code = Menu_Report_Console( &p );
//		return error_code;
//	}
	...
	error_code = Menu_Periodic_Run( ds->main_loop_time_sec,
									Tracere_Simple_Main_Loop,
									Tracere_Simple_Main_Stop_Request,
									ds
								  );
	while(1)
	{
